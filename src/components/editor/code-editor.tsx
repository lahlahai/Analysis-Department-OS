...
export function CodeEditor({ path, value, onChange }: CodeEditorProps) {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-[#0d131c]" dir="ltr">
    ...
    <div className="min-h-0 flex-1" dir="ltr"><MonacoEditor value={value} onChange={(next) => onChange(next ?? "")} language={language} theme="light" options={{ automaticLayout: true, minimap: { enabled: false }, lineNumbers: "on", lineNumbersMinChars: 3, glyphMargin: false, fontSize: 12, lineHeight: 21, padding: { top: 16 }, tabSize: 2, wordWrap: "on", scrollBeyondLastLine: false, renderLineHighlight: "gutter" }} /></div>
  </div>;
}
