...
function citizenServiceTableDocument(service: CitizenService) {
  ...
  return `${JSON.stringify({ version: "1.0", serviceId: service.id, request: serviceWithSpec, specification: spec, userStory: spec?.userStory, links, columns: ["البيان", "التفاصيل"], rows, columnWidths: [220, 680], rowHeights: rows.map(() => 42) }, null, 2)}\n`;
}
